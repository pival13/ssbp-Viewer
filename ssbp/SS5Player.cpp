...
public:
    ...
    bool setCellRefTexture(const ProjectData* data, const char* cellName, long texture)
    {
        ...
        for (int i = 0; i < data->numCells; i++)
        {
            ...
            if (strcmp(cellName, name) == 0)
            {
                ...
                int w, h;
                SSGetTextureSize(texture, w, h);
                ref->texture.size_w = w;
                ref->texture.size_h = h;
                rc = true;
            }
        }
    ...
    std::vector<std::string> getTextureName() const { return _texname; }
    TextuerData &getTextureData(const std::string &texName)
    {
        for (size_t i = 0; i != _texname.size(); ++i)
            if (_texname[i] == texName) return _textures[i];
        SS_ASSERT2(1, "Unknow texture " + texName);
    }
...
void Player::getCanvasSize(size_t *sizeX, size_t *sizeY)
{
    *sizeX = _currentAnimeRef->animationData->canvasSizeW;
    *sizeY = _currentAnimeRef->animationData->canvasSizeH;
}
...
float Player::parcentValRot(float val1, float val2, float parcent)
{
    ...
    else
    {
        ...
        {
            int isa = 3600 - islr;
            inewval = int(ival2 - ((float)isa * parcent));
        }
        else
        {
            int isa = islr;
