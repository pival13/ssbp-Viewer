...
struct State
{
    int flags;                      /// Status flag for a specific frame
    int cellIndex;                  /// Cell index assigned to the part
    float x;                        /// SS5 attribute: X coordinate
    float y;                        /// SS5 attribute: Y coordinate
    float z;                        /// SS5 attribute: Z coordinate
    float pivotX;                   /// X offset + X offset in the cell
    float pivotY;                   /// Y offset + Y offset in the cell
    float rotationX;                /// Relative X rotation
    float rotationY;                /// Relative Y rotation
    float rotationZ;                /// Relative Z rotation
    float scaleX;                   /// Relative X scale
    float scaleY;                   /// Relative Y scale
    int opacity;                    /// Relative opacity (0-255)
    float size_X;                   /// SS5 attribute: X size
    float size_Y;                   /// SS5 attribute: Y size
    float uv_move_X;                /// SS5 attribute: UV X offset
    float uv_move_Y;                /// SS5 attribute: UV Y offset
    float uv_rotation;              /// SS5 attribute: UV rotation
    float uv_scale_X;               /// SS5 attribute: UV X scale
    float uv_scale_Y;               /// SS5 attribute: UV Y scale
    float boundingRadius;           /// SS5 attribute: curve radius
    int colorBlendFunc;             /// SS5 attribute: how to blend color
    int colorBlendType;             /// SS5 attribute: カラーブレンドの単色か頂点カラーか。
    bool flipX;                     /// Relative horizontal flip
    bool flipY;                     /// Relative vertical flip
    bool isVisibled;                /// Relative visibility
    ...
    int blendfunc;                  /// Blending method for parts
    float mat[16];                  /// Relativ transformation matrix
...
public:
    ...
    */
    ...
    int getAnimFps() const { return _animefps; }
    ...
    */
    ...
    void getCanvasSize(size_t *sizeX, size_t *sizeY);
